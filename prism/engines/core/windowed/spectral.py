...
class SpectralFeatures:
    ...
    # Phase spectrum (extracted via FFT)
    phase_at_dominant_freq: Optional[float]  # Phase at dominant frequency (radians)
    phase_mean: Optional[float]              # Mean phase across spectrum (radians)
    phase_std: Optional[float]               # Phase std across spectrum (radians)
...
def compute_spectral_features(
    ...
    if n < 16:
        return SpectralFeatures(
            ...
            phase_at_dominant_freq=None, phase_mean=None, phase_std=None
        )
    ...
    # --- Phase spectrum extraction (via direct FFT) ---
    # Welch returns PSD only; we need raw FFT for phase
    fft_vals = np.fft.rfft(values_detrend)
    phase = np.angle(fft_vals)

    # Skip DC component for phase analysis (index 0)
    phase_no_dc = phase[1:] if len(phase) > 1 else phase

    # Phase at dominant frequency
    # Map dominant_freq back to FFT bin (different from Welch bin)
    fft_freqs = np.fft.rfftfreq(len(values_detrend), d=1.0)
    fft_freqs_no_dc = fft_freqs[1:] if len(fft_freqs) > 1 else fft_freqs

    # Find closest FFT bin to dominant frequency
    if len(fft_freqs_no_dc) > 0 and dominant_freq > 0:
        fft_dominant_idx = np.argmin(np.abs(fft_freqs_no_dc - dominant_freq))
        phase_at_dominant_freq = float(phase_no_dc[fft_dominant_idx])
    else:
        phase_at_dominant_freq = 0.0

    phase_mean = float(np.mean(phase_no_dc)) if len(phase_no_dc) > 0 else 0.0
    phase_std = float(np.std(phase_no_dc)) if len(phase_no_dc) > 0 else 0.0
    ...
    return SpectralFeatures(
        ...
        phase_at_dominant_freq=phase_at_dominant_freq,
